...
ias_rms = float(np.sqrt(np.mean(res["ias"][-ss:]**2))) / k
ibs_rms = float(np.sqrt(np.mean(res["ibs"][-ss:]**2))) / k
ics_rms = float(np.sqrt(np.mean(res["ics"][-ss:]**2))) / k
...
eta = (P_mec / P_gap * 100) if P_gap > 0 else 0.0
