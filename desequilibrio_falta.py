...
def abc_voltages_deseq(t: np.ndarray, Vl: float, f: float,
                       deseq_a: float = 0.0,
                       deseq_b: float = 0.0,
                       deseq_c: float = 0.0,
                       falta_fase_a: bool = False,
                       falta_fase_b: bool = False,
                       falta_fase_c: bool = False) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """
    tetae: np.ndarray = 2.0 * np.pi * f * t
    zero = np.zeros_like(t)

    Va: np.ndarray = zero if falta_fase_a else np.sqrt(2.0) * Vl * (1.0 + deseq_a) * np.sin(tetae)
    Vb: np.ndarray = zero if falta_fase_b else np.sqrt(2.0) * Vl * (1.0 + deseq_b) * np.sin(tetae - 2.0 * np.pi / 3.0)
    Vc: np.ndarray = zero if falta_fase_c else np.sqrt(2.0) * Vl * (1.0 + deseq_c) * np.sin(tetae + 2.0 * np.pi / 3.0)
